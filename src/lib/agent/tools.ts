...
export const TOOL_DEFS: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      "Cut the subject out of an image layer, erasing its background to transparency (turns a boxy image into a clean sticker-style cutout). Two engines: a pixel-exact flood fill for flat single-color baked backgrounds, and an in-browser AI segmentation model for complex ones (photos, gradients, painted backdrops, scenery). mode 'auto' (default) runs flood fill and escalates to AI when the border doesn't peel cleanly; the returned `method` says which ran. IMPORTANT: flood fill only judges the image BORDER — if the next screenshot still shows leftover backdrop AROUND the subject (e.g. a painted circle behind a logo), call this ONCE more with mode 'ai'; never loop beyond that. Use mode 'ai' directly for photographic subjects on real scenes. AI keeps the subject but recuts edges; flood is byte-exact on the subject — prefer flood for pixel-art/graphics with flat backgrounds. First AI use downloads a model (~tens of MB, then cached) and can take a while.",
    inputSchema: {
      type: "object",
      properties: {
